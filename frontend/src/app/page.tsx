  ...
  return (
    <main className="relative min-h-screen flex flex-col items-center justify-center px-6 py-16">
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-purple-500/10 rounded-full blur-3xl animate-[pulse_8s_ease-in-out_infinite]" />
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-blue-500/10 rounded-full blur-3xl animate-[pulse_12s_ease-in-out_infinite_2s]" />
      </div>
      ...
      <div className="relative mb-10 flex items-center gap-2 px-4 py-2 rounded-full border border-purple-800 bg-purple-950/40 text-purple-300 text-sm font-medium">
        <span className="w-2 h-2 rounded-full bg-purple-400 animate-pulse" />
        HCLTech x OpenAI Agentic AI Hackathon — Track 2: Sales Operations
      </div>

      <div className="relative text-center mb-12 space-y-4 max-w-3xl">
        ...
        <p className="text-3xl text-white font-semibold mt-4">
          From RFP to Winning Proposal in Minutes
        </p>
        <p className="text-lg text-gray-400 font-light max-w-2xl mx-auto">
          AI-powered pursuit intelligence that surfaces hidden disqualifiers, simulates competitor bids, and generates evidence-based proposals
        </p>
      </div>

      {/* Stat cards */}
      <div className="relative grid grid-cols-3 gap-4 mb-10 max-w-xl w-full">
        <div className="text-center bg-gray-900/60 border border-gray-800 rounded-xl p-4">
          <p className="text-2xl font-bold text-purple-400">Weeks &rarr; Minutes</p>
          <p className="text-xs text-gray-500 mt-1">Pursuit Cycle Time</p>
        </div>
        <div className="text-center bg-gray-900/60 border border-gray-800 rounded-xl p-4">
          <p className="text-2xl font-bold text-red-400">Hidden Risks</p>
          <p className="text-xs text-gray-500 mt-1">Detected Automatically</p>
        </div>
        <div className="text-center bg-gray-900/60 border border-gray-800 rounded-xl p-4">
          <p className="text-2xl font-bold text-cyan-400">Live Intel</p>
          <p className="text-xs text-gray-500 mt-1">Market Intelligence</p>
        </div>
      </div>
      ...
      <div className="relative w-full max-w-xl space-y-4">
        ...
        <button
          onClick={async () => {
            setLoading(true);
            try {
              const res = await fetch(`${API}/api/pursuit/instant-demo`);
              if (res.ok) {
                router.push(`/pursuit/RFP-FB30BB`);
              } else {
                setError("No pre-computed demo available.");
                setLoading(false);
              }
            } catch {
          ...
          className="w-full py-3 rounded-xl font-medium text-sm transition-all
                     border border-gray-700 hover:border-purple-500 bg-gray-900/60 hover:bg-gray-800
      ...
      <div className="relative mt-16 max-w-3xl w-full">
