...
def run_codex(prompt: str, timeout: int = 180) -> str | None:
    ...
    if STREAM_ENABLED:
        return _run_streaming(cmd, timeout)
    else:
        return _run_batch(cmd, timeout)


def _run_batch(cmd: list, timeout: int) -> str | None:
...
def _run_streaming(cmd: list, timeout: int) -> str | None:
...
def is_codex_available() -> bool:
    ...
    try:
        result = subprocess.run(
            [str(codex), "exec", "--json", "--ephemeral", "-s", "read-only", "Reply: OK"],
            capture_output=True,
            text=True,
            timeout=30,
            input="",
