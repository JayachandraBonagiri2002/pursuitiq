...
def run_win_intel(decomposition: RFPDecomposition, vector_store_id: str) -> WinIntelResult:
    ...
    # ── Data Quality Gate: check how many layers have real data ───────────────
    layer_count = 3
    quality_warnings = []

    if "not yet populated" in knowledge_context.lower() or not knowledge_context.strip():
        layer_count -= 1
        quality_warnings.append("Knowledge base has no relevant past proposals for this deal type")

    if "unavailable" in procurement_context.lower() or not procurement_context.strip():
        layer_count -= 1
        quality_warnings.append("Public procurement data unavailable for this geography/industry")

    if "not available" in corpus_context.lower() or "no relevant" in corpus_context.lower() or not corpus_context.strip():
        layer_count -= 1
        quality_warnings.append("Deal corpus returned no relevant similar deals")

    data_quality_warning = ""
    if layer_count < 2:
        data_quality_warning = (
            "\n\nWARNING: Limited intelligence available (only "
            f"{layer_count}/3 data layers returned real data). "
            "Cap win_probability confidence at 0.3 and clearly state which data sources "
            "are missing. Missing sources: " + "; ".join(quality_warnings)
        )
        logger.warning(f"Agent 2: data quality gate triggered — only {layer_count}/3 layers have data")

    logger.info(f"Agent 2: all three layers gathered ({layer_count}/3 with real data), synthesising...")
    ...
    if result is None:
        raise ValueError("Agent 2 (Win Intel) returned no parseable output - possible refusal")
