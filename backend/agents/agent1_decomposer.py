...
DEAL SIZE AND DURATION EXTRACTION:
You MUST extract estimated_deal_size_usd and contract_duration. Look for:
- Explicit budget ranges ("estimated value EUR 45-65M", "not to exceed $10M")
- Current spend indicators ("current annual IT spend of $X", "existing contract valued at $X")
- Indirect signals: number of FTEs mentioned, number of servers/workloads, data volume
- Duration: contract term, renewal options, "initial period of X years"
- If not explicitly stated, INFER from scope: count requirements, estimate complexity,
  and provide a reasonable range (e.g., "$5M-15M over 3 years based on scope")
- NEVER leave both fields as null. At minimum infer from the scope described.
  A hosting/infrastructure RFP with 50+ requirements = minimum $5M deal.
  An enterprise transformation with 100+ requirements = minimum $15M deal.
...
def decompose_rfp(rfp_text: str, rfp_id: str | None = None) -> RFPDecomposition:
    ...
    if result is None:
        raise ValueError("Agent 1 (Decomposer) returned no parseable output - possible refusal")
