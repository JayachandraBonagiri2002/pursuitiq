...
def run_competitor_shadow(
    ...
    competitors_override: list[str] | None = None,
) -> CompetitorShadow:
    """
    Agent 4: Competitor War Room — multi-source competitive intelligence.

    AGENTIC: When competitors_override is provided (from Planner Agent),
    uses AI-identified competitors instead of the hardcoded fallback list.
    This makes the competitor research adaptive to each specific RFP.
    """
    client = get_client()

    # AGENTIC: Use planner-identified competitors if available, else fallback
    competitors = competitors_override if competitors_override else MAJOR_COMPETITORS
    competitor_source = "planner_agent" if competitors_override else "hardcoded_fallback"
    ...
    logger.info(
        f"Agent 4: building competitor war room | rfp_id={decomposition.rfp_id} | "
        f"competitors={competitors} | source={competitor_source}"
    )
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = [executor.submit(_search, c) for c in competitors]
    ...
    # ── Source 2: Public procurement data (general context only — specifics from web) ─
    procurement_intel = ""
    try:
    ...
    if result is None:
        raise ValueError("Agent 4 (Competitor Shadow) returned no parseable output - possible refusal")
    logger.info(
