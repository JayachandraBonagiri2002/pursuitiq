...
def _generate_bulk(n: int = 80) -> list[dict]:
    ...
    # Diverse, specific, actionable lessons for bulk deals
    win_lessons = [
        "Won by proposing a phased migration that reduced client risk vs competitor's big-bang approach",
        "Lost initially but won on recompete by demonstrating 99.99% SLA track record from previous engagement",
        "Key differentiator was offering a dedicated innovation lab co-located with client's R&D team",
        "Won on price after demonstrating 40% TCO reduction through automation over 5-year period",
        "Competitor had better tech but we won by embedding a change management team that addressed adoption risk",
        "Won because we were the only bidder who proposed a gainshare model tied to business outcomes",
        "Pre-existing relationship with client's procurement director gave us insight into unstated evaluation criteria",
        "Won by including a free 3-month proof of concept that de-risked the decision for the client",
        "Demonstrated 25% faster time-to-value with pre-built industry accelerators vs competitor's custom build",
        "Won by committing named senior architects for the full engagement instead of generic resource pools",
        "Client chose us because we brought a reference customer from same industry who spoke at the orals presentation",
        "Won by identifying a $12M hidden cost in the incumbent's renewal proposal and offering transparent pricing",
        "Proposed a dual-vendor strategy where we took the complex work and subcontracted commodity tasks, beating single-vendor bids on flexibility",
        "Won by mapping our delivery methodology to the client's existing PMO framework, reducing integration friction",
        "Key win factor was offering contractual penalties tied to delivery milestones — competitors only offered best-effort SLAs",
        "Won by demonstrating a working prototype during the orals presentation while competitors showed only PowerPoints",
        "Client valued our proposal because every team member had active security clearances, avoiding 6-month onboarding delays",
        "Won by identifying regulatory changes 18 months out and building compliance into the architecture proactively",
        "Differentiated by offering 24/7 follow-the-sun support model across 3 time zones at no additional cost",
        "Won because we proposed reusing 60% of the client's existing codebase rather than a full rewrite, saving $8M",
        "Key factor was our willingness to accept liability caps that matched client's insurance requirements — competitors refused",
        "Won by proposing an open-source stack that eliminated $4M/year in licensing fees the client was paying to incumbent",
        "Client chose us after we identified 3 critical gaps in the RFP scope that competitors ignored, showing deeper understanding",
        "Won by partnering with a niche local firm for regulatory expertise instead of trying to cover it alone",
        "Decisive factor was our automated testing framework that guaranteed 95% test coverage from day one",
        "Won by structuring payment milestones around business value delivered rather than time-and-materials billing",
        "Client selected us because our transition plan had zero planned downtime vs competitor's 72-hour migration window",
        "Won because we proposed keeping 40% of the incumbent's staff through TUPE transfer, reducing client's political risk",
        "Key differentiator was real-time project dashboard giving client CIO visibility without status meetings",
        "Won by offering a contractual right-to-audit clause and quarterly independent quality assessments",
    ]

    loss_lessons = [
        "Missed local entity requirement in compliance appendix — discovered too late to establish subsidiary",
        "Incumbent vendor relationship was too strong to displace — they had embedded staff in client's org for 8 years",
        "Pricing 20% above winning bid despite better solution — client weighted price at 40% and we underestimated it",
        "Could not meet mandatory nationality staffing ratio required by government procurement regulations",
        "Lacked required certification discovered late in process — 6 weeks of bid work wasted",
        "Failed to address a key evaluation criterion that was worth 25% of total score — lost by 3 points",
        "Proposed team leads were double-booked on another engagement and client discovered this during reference checks",
        "Our solution was over-engineered for client's actual needs — simpler competitor bid scored higher on feasibility",
        "Lost because we could not provide three references from the same industry vertical as required",
        "Client's evaluation panel changed mid-process and new CTO preferred competitor's technology stack",
    ]
    ...
    for i in range(21, 21 + n):
        ...
        if out == "WON":
            ...
            deal["lesson"]      = win_lessons[i % len(win_lessons)]
        else:
            deal["loss_reason"] = random.choice(loss_r)
            deal["lesson"]      = loss_lessons[i % len(loss_lessons)]
